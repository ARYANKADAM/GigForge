...
export async function POST(req) {
  ...
  if (evt.type === 'user.created' || evt.type === 'user.updated') {
    ...
    await User.findOneAndUpdate(
      { clerkId: id },
      { clerkId: id, email: email_addresses[0]?.email_address, firstName: first_name || '', lastName: last_name || '', imageUrl: image_url, role: unsafe_metadata?.role || 'client' },
      { upsert: true }
    )
  }
  if (evt.type === 'user.deleted') {
    await User.findOneAndUpdate({ clerkId: evt.data.id }, { isBanned: true })
  }
